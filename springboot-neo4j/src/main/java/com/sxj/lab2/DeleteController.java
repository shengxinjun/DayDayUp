	...
	public void doDelete(){
		...
		try {
			...
			for(PostPersonRelation relation:relations){
				session.run(
						"match(pe:Person{id:{personId}})-[r]->(po:Post{id:{postId}}) delete r",
						Values.parameters("personId", relation.getPersonId(), "postId", relation.getPostId()));
				
			}
			for(Person person:persons){
				session.run(
						"match(pe:Person{id:{personId}}) delete pe",
						Values.parameters("personId", person.getId()));
			}
			for(Post post:posts){
				session.run(
						"match(po:Post{id:{postId}}) delete po",
						Values.parameters("postId", post.getId()));
				
			}
			endTimeString=DateUtil.currentDateTime();
			long endTime = System.currentTimeMillis();
			String msg="删除10w条数据用时：" + (endTime - startTime)+" ms,开始时间："+startTimeString+",结束时间："+endTimeString;
